...
`data/real_attachments/`。代码按下列相对位置读取，问题一、问题二、问题四读的是**同一份** `data/`。
...
└─ data/                                  # 题目附件解压到这里（不随仓库分发）
   └─ real_attachments/
      └─ A_data_value/
         ├─ slimpajama_quality_signal_sample.jsonl      # 单文件；另有同名的 .xz 压缩版
         ├─ slimpajama_quality_extended/
         │  ├─ arxiv_part-6777d8857c6e-000486.jsonl      # 同样附有 .xz 压缩版
...
`Q1_1` 的 `locate_a1()` 会自行在 `data/real_attachments/A_data_value/` 下递归查找质量信号文件，
**`.jsonl` 与 `.jsonl.xz` 两种形式都能读**；只解压其中一种即可（解压后的
`slimpajama_quality_signal_sample.jsonl` 约 413 MB，压缩版约 99 MB，磁盘紧张时保留 `.xz` 即可）。
`Q1_2`、`Q1_3` 读的 `slimpajama_quality_extended/` 与 `regmix_tables/` 同理。

不要把附件复制进 `Q1/`，也不要把生成的 `results/` 提交到 GitHub。若改变上述目录层级，
需要同步修改 `Q1/Q1_1/Q_1_1_1_audit_A1.py`、`Q1/Q1_1/Q_1_1_6_validate_extensions.py`、
`Q1/Q1_2/Q_1_2_1_prepare_signals.py`、`Q1/Q1_2/Q_1_2_9_text_cases.py` 和
`Q1/Q1_3/Q_1_3_1_load_and_standardize.py` 顶部的数据路径。
