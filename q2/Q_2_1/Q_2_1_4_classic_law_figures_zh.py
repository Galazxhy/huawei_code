...
``steps-b-b2``
    同上，但数据源换成数据集 B2（Cerebras 训练日志），7 个模型规模。
...
用法::
    ...
    python Q2/Q_2_1/Q_2_1_4_classic_law_figures_zh.py
    python Q2/Q_2_1/Q_2_1_4_classic_law_figures_zh.py --figures extra-a,validation-a
"""
...
BUNDLED_FONTS = (
    _q2_paths.PROJECT_ROOT / "assets" / "NotoSansSC-Regular-subset.otf",
    _q2_paths.PROJECT_ROOT / "assets" / "NotoSansSC-Bold-subset.otf",
)
# 仓库统一的中文字体（`.fontwork/` 下，由 Q_2_2_2_generalized_law_figures.chinese_family 使用）。
# 自带子集字体随 `Q2/assets/` 一起清理后，中文靠它渲染；两个字重的子集在时优先用子集。
FALLBACK_FONTS = (
    _q2_paths.REPO_ROOT / ".fontwork" / "NotoSansCJKsc-Regular.otf",
)
# 实际参与渲染的字体路径（用于字形覆盖检查）。
FONT_PATHS = BUNDLED_FONTS + FALLBACK_FONTS
DEFAULT_RESULT_DIR = (
    _q2_paths.ANALYSIS_DIR / "traditional_scaling_law"
)
DEFAULT_FULL_DIR = _q2_paths.ANALYSIS_DIR / "scaling_law_full"
...
def configure_chinese() -> tuple[str, bool]:
    ...
    if bundled == 1:
        print(
            ...
            "      请运行 python Q2/figure_font_subset.py 生成两个字重。",
            file=sys.stderr)
    if bundled == 0:
...
def assert_glyphs_available(font_paths: tuple[Path, ...]) -> None:
    ...
    if missing:
        raise RuntimeError(
            ...
            f"请重新运行 python Q2/figure_font_subset.py 生成子集。"
        )
...
#   经典标度律拟合参数
#       results/traditional_scaling_law/traditional_scaling_fit.json
#   经典律残差点
#       results/traditional_scaling_law/traditional_scaling_predictions.csv
#   早停历史 / 跨族验证点 / 验证指标
#       results/scaling_law_full/{early_stopping_history,
#           early_stopping_predictions,classic_validation_metrics}.csv
#       results/scaling_law_full/scaling_law_full_results.json
# --------------------------------------------------------------------------- #
...
class _DataAccess:
    ...
    FULL_DIR = _q2_paths.ANALYSIS_DIR / "scaling_law_full"
    DEFAULT_TRAINING_LOG = (_q2_paths.B_DATA /
                            "pythia_training_log_existing.csv")
    DEFAULT_B2_TRAINING_LOG = (_q2_paths.B_DATA /
                               "cerebras_training_log.csv")
    DEFAULT_FIT_JSON = (_q2_paths.ANALYSIS_DIR /
                        "traditional_scaling_law" / "traditional_scaling_fit.json")
    DEFAULT_PREDICTIONS = (_q2_paths.ANALYSIS_DIR /
...
def figure_validation_a() -> plt.Figure:
    ...
    # 数据是一条沿对角线铺开的带，把左上和右下角都占了；轴内整块空白只有对角线
    # 下方的一小块三角区，四个数据源 + 两条参考线共 6 个图例条目塞不进去
    # （0 遮挡要求下，无论单列还是字号 10 都会压住散点）。因此把图例移到坐标轴
    # 上方，`assert_legend_clear` 只统计轴内图例，轴外图例天然不可能遮挡数据。
    axis.legend(loc="lower left", bbox_to_anchor=(0.0, 1.02), ncol=2,
                title="样本数，中位相对误差", framealpha=1.0,
    ...
    for name in names:
        ...
        for extension in formats:
            ...
            figure.savefig(
                ...
                metadata={
                    "Title": basename,
                    "Creator": "Q_2_1_4_classic_law_figures_zh.py",
                })
            outputs.append(path)
        plt.close(figure)
