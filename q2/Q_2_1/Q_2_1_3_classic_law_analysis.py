...
基准律，但数据、参数与产物目录都相互独立：
    ...
    Q2/Q_2_2/Q_2_2_1_generalized_law.py    # (E_k, A_k, B_k, eta_k, zeta_k, Psi_k) 辨识
    Q2/Q_2_2/Q_2_2_2_generalized_law_figures.py   # 出图

**B6--B8 不参与任何参数估计**，仅在需要时作为事后对照。

数据集 B 的实验与数据集 A 相互独立，且 B7 为半合成数据，本程序不使用 B7 参数替换 B1
的真实数据参数。

默认运行：

    python Q2/Q_2_1/Q_2_1_3_classic_law_analysis.py

结果写入 ``results/scaling_law_full/``。
"""
...
from Q_2_1_1_classic_law import (
    FitResult,
    Observation,
    ScalingParameters,
    fit_scaling_law,
)
from Q_2_1_2_classic_law_early_stopping import (
    MODEL_PATH,
    CandidateFit,
    candidate_payload,
...
DEFAULT_B_DIR = _q2_paths.B_DATA
DEFAULT_OUTPUT_DIR = _q2_paths.ANALYSIS_DIR / "scaling_law_full"
...
def main() -> int:
    ...
    if not args.disable_early_stopping:
        ...
        early_output_files = {
            "early_stopping_history": _q2_paths.repo_relative(history_path),
            "adaptive_weights": _q2_paths.repo_relative(weights_path),
            "early_stopping_predictions": _q2_paths.repo_relative(predictions_path),
        }
    ...
    payload: dict[str, object] = {
        "methodology": {
            "Q_2_1_1_classic_law": "L=E+A*N^(-alpha)+B*D^(-beta)",
            "scope_note": (
                "本脚本只辨识经典标度律，自变量为参数量 N 与训练数据量 D。"
                "数据质量 Q 与配比 p 由独立的广义标度律脚本引入，"
                "见 Q2/Q_2_2/Q_2_2_1_generalized_law.py（其辨识数据限定为 "
                "B1 + 数据集 A + Q_1_to_2，B6--B8 不参与参数估计）。"
            ),
        },
        "data_credibility": {
            "B1": "real, main fit",
        ...
        "output_files": {
            "large_model_extrapolation": _q2_paths.repo_relative(large_output_path),
            **early_output_files,
        },
    }
    ...
    payload["output_files"]["validation_metrics"] = _q2_paths.repo_relative(validation_path)
    payload["output_files"]["full_results"] = _q2_paths.repo_relative(result_path)
    payload["output_files"]["markdown_report"] = _q2_paths.repo_relative(report_path)
    result_path.write_text(
        json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8"
    )
