...
与 ``Q_2_2_1_generalized_law.py`` 的辨识结果逐位一致（偏差 < 1e-14），
只是换了记号：把 ``N_ref``、``D_ref`` 吸收进配比项，并把 ``alpha = beta`` 合并。

产出::

    results/Q2_to_Q3/广义标度律_论文写法.md
    results/Q2_to_Q3/generalized_scaling_law_paper_coefficients.csv

用法::

    MPLCONFIGDIR=$PWD/.mplcfg python Q2/Q_2_3/Q_2_3_2_handoff_paper_form.py
"""
...
def collect() -> dict:
    ...
    law = json.loads(
        (_q2_paths.HANDOFF_IN / "generalized_scaling_compact.json")
...
def markdown(values: dict, gap: float) -> str:
    ...
    add("本文件由 `Q2/Q_2_3/Q_2_3_2_handoff_paper_form.py` 从 `results/Q_1_to_2/"
    ...
    add("2. **$Q_i$ 不可独立辨识**。数据集 A 中每个域的 $Q_i$ 固定，自由域系数可以把它的尺度整体吸收，"
...
def main() -> None:
    ...
    print(f"wrote {_q2_paths.repo_relative(MARKDOWN)}")
    print(f"wrote {_q2_paths.repo_relative(COEFFICIENTS)} "
