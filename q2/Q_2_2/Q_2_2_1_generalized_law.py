...
HANDOFF = _q2_paths.HANDOFF_IN
CLASSIC = _q2_paths.CLASSIC_RESULTS
OUTPUT = HANDOFF / "generalized_scaling_compact.json"
REPORT = _q2_paths.ANALYSIS_DIR / "generalized_scaling_law" / "generalized_scaling_law_compact_report.md"
SCALES = (
...
def run(regmix: Path = REGMIX, handoff: Path = HANDOFF, classic_path: Path = CLASSIC,
    ...
    return {
        ...
        "sources": {
            "quality": _q2_paths.repo_relative(handoff / "Q17_mapping.csv"),
            "exponents": _q2_paths.repo_relative(classic_path) + " : B1_main_fit.parameters",
            "blocks": [
                {"name": block.name, "mixture_file": _q2_paths.repo_relative(regmix / block.mixture_file),
                 "loss_file": _q2_paths.repo_relative(regmix / block.loss_file), "n_mixtures": len(block.ids),
...
def write_markdown_report(result: dict[str, object], path: Path) -> None:
    ...
    add("")
    add("> 由 `Q2/Q_2_2/Q_2_2_1_generalized_law.py` 自动生成，数据源为数据集 A 的配比实验、")
    ...
    add("3. **指数固定为 B1 的值**——数据集 A 只有 3 个不同的 (N, D) 点，"
    ...
    add("- **指数借用自 B1**：数据集 A 的三个 (N, D) 点使标准标度律恰好饱和，"
    ...
    add("")
    add("```bash")
    add("python Q2/Q_2_2/Q_2_2_1_generalized_law.py \\")
    add("    --regmix-dir data/real_attachments/A_data_value/regmix_tables \\")
    add("    --handoff-dir results/Q_1_to_2 \\")
    add("    --classic-results results/scaling_law_full/scaling_law_full_results.json \\")
    add("    --output results/Q_1_to_2/generalized_scaling_compact.json \\")
    add("    --report results/generalized_scaling_law/generalized_scaling_law_compact_report.md")
    add("```")
    add("")
