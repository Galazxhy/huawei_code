...
1. ``N``、``D`` 的解析导数来自交付模型本体，并用中心差分复核（不通过即报错）；
   ``p`` 的可行方向导数同样与差分核对（复用 ``elasticity_analysis.check_derivative``）。
2. ``Q`` **不能**由广义标度律识别：每个训练域的 ``Q_i`` 在附件 A 中固定，模型只通过
...
默认运行：
    python q2/factor_elasticity_analysis.py
产物写入 ``data_analysis/factor_marginal_elasticity/``。
"""
...
OUTPUT = ROOT / "data_analysis" / "factor_marginal_elasticity"
...
def write_markdown(result: dict, path: Path) -> None:
    ...
    lines = [
        "# 各因素边际效用与弹性",
        "",
        "本文件由 `q2/factor_elasticity_analysis.py` 生成。",
        "",
        "## 0. 口径",
        "",
        "| 量 | 定义 |",
