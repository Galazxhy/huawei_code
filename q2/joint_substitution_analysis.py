...
默认运行：
    python q2/joint_substitution_analysis.py
产物写入 ``data_analysis/joint_quality_mixture_substitution/``。
"""
...
OUTPUT = ROOT / "data_analysis" / "joint_quality_mixture_substitution"
sys.path.insert(0, str(Path(__file__).resolve().parent))
sys.path.insert(0, str(HANDOFF))

import elasticity_analysis as ea  # noqa: E402
import generalized_law as compact  # noqa: E402
import quality_substitution_analysis as qsub  # noqa: E402
...
def write_markdown(payload: dict, path: Path) -> None:
    ...
    lines += [
        ...
        "- 横轴覆盖质量实验标定过的整个 $Q$ 区间 $[0.1,1.0]$（步长 0.02，包含 $Q_0$、$Q_1$）：$Q_0=0.6$、$Q_1=0.7$ 是观测锚点，区间两端的曲线是同一外部标定 $\\gamma$ 下的情景推算（$Q$ 维本就由独立质量实验标定，不在附件 A 的观测范围内）；只画 $[0.5,1.0]$ 时所需 Token 仅变 17%、幂律近乎直线，看不出「质量越低、提升越值钱」的弯；",
    ...
    lines += [
        ...
        "（脚本 `q2/joint_substitution_figures.py` 的 `cylinder_figure`）。",
        "",
        "**布局与顺序**",
        "",
        ...
        "",
        "**三维绘制要点**（沿用既有图集 `elasticity_figures.py` 的做法）",
        "",
