...
默认运行：
    python q2/quality_substitution_analysis.py
产物写入 ``data_analysis/quality_token_substitution/``。
"""
...
DESIGN = (ROOT / "data" / "real_attachments" / "B_scaling_laws"
          / "supplementary_NQ_experiment_expanded.csv")
HANDOFF = ROOT / "data_analysis" / "Q2_to_Q3"
OUTPUT = ROOT / "data_analysis" / "quality_token_substitution"
...
def analyse() -> dict:
    ...
    correction = {
        ...
        "handoff_material": "data_analysis/Q2_to_Q3/交付文档.md §4.3",
...
def write_markdown(payload: dict, path: Path) -> None:
    ...
    lines += [
        ...
        "```bash",
        "python q2/quality_substitution_analysis.py",
        "python q2/quality_substitution_figures.py",
        "```",
        "",
    ]
    path.parent.mkdir(parents=True, exist_ok=True)
