...
def main() -> None:
    ...
    # Compare on the calendar day, not on the timestamp.  The cutoff is stated
    # as the end of 2025-03-31, and the frontier's own timestamps carry
    # nanosecond precision (…23:59:59.999999999), which is *after* the
    # microsecond-precision constant even though it is the same instant.
    cutoff_day = ANALYSIS_END.normalize()
    assert frontier["date"].dt.normalize().max() <= cutoff_day
    assert compute["date"].dt.normalize().max() <= cutoff_day
