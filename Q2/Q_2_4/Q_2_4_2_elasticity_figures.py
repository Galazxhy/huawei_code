...
DEFAULT_INPUT = _q2_paths.ANALYSIS_DIR / "quality_mixture_elasticity"
sys.path.insert(0, str(_q2_paths.HANDOFF_OUT))
...
def register_cjk() -> str:
    ...
    字体查找交给仓库统一入口 ``Q_2_2_2_generalized_law_figures.chinese_family``：它按
...
@contextmanager
def panel_style():
    """Canvas and font sizes of ``Q_2_1_4_classic_law_figures_zh.py``.
...
SHARE_STEP = 0.01
# 与 Q_2_4_1_elasticity_analysis.py 一致的支持域筛选口径。
...
def predicted_gain_estimates(step: float = SHARE_STEP,
    ...
    Mirrors ``Q_2_4_1_elasticity_analysis.finite_share_gain``: for every
    ...
    law = GeneralizedLawEvaluator.from_directory(_q2_paths.HANDOFF_OUT)
