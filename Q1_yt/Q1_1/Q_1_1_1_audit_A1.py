...
ATTACHMENT_DIR = PROJECT_DIR.parents[1] / "F 题" / "F题" / "real_attachments" / "A_data_value"
...
def locate_a1() -> Path:
    candidates = list(ATTACHMENT_DIR.rglob("slimpajama_quality_signal_sample.jsonl"))
    candidates += list(ATTACHMENT_DIR.rglob("slimpajama_quality_signal_sample.jsonl.xz"))
    candidates = [path for path in candidates if path.is_file()]
    if len(candidates) != 1:
        raise FileNotFoundError(f"Expected exactly one A1 JSONL file; found {len(candidates)}: {candidates}")
    return candidates[0]
